...
export function Musics() {
  ...
  useEffect(() => {
    const displayDuration = 3000 // 3 seconds for the current set to be fully visible
    const blurTransitionDuration = 500 // Matches the CSS `duration-500` for the blur effect
    const totalCycleTime = displayDuration + blurTransitionDuration // Total time for one complete cycle

    const interval = setInterval(() => {
      // Step 1: After `displayDuration`, start the blur transition
      setTimeout(() => {
        setIsTransitioning(true)
      }, displayDuration)

      // Step 2: After `totalCycleTime`, change the content and remove the blur
      setTimeout(() => {
        setCurrentSetIndex((prevIndex) => (prevIndex + 1) % albums.length)
        setIsTransitioning(false)
      }, totalCycleTime)
    }, totalCycleTime) // The entire cycle repeats every `totalCycleTime`
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      <div
        className={`relative w-[680px] h-[200px] transition-all duration-500 ${
      ...
      >
        {currentAlbums.map((album) => (
          <div
            key={album.url}
            className="absolute w-23 h-23 rounded-xl border-3 border-white dark:border-neutral-800 shadow-lg transition-all duration-500 ease-in-out cursor-pointer hover:scale-102"
            style={{ transform: album.position, zIndex: album.zIndex, top: "50%", left: "50%" }}
        ...
        {albums.map((_, index) => (
          <div
            ...
            className={`w-1 h-1 rounded-full transition-all duration-300 ${
              index === currentSetIndex ? "bg-neutral-500 scale-125" : "bg-neutral-300 dark:bg-neutral-600"
            }`}
          />
        ))}
      </div>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="text-center">
                  <Link href={selectedAlbum.url} target="_blank" rel="noopener noreferrer">
                    <Button className="mt-4">Listen on Spotify</Button>
                  </Link>
                </div>
              </div>
            </>
